...
import type {
  SlopInput,
  SlopResult,
} from "@faerrin/heartwood/src/eval/slop.ts";
...
export const getDashboard = createServerFn({ method: "GET" }).handler(
  async (): Promise<DashboardData> => {
      ...
      for (const name of names) {
        ...
        try {
          const row = JSON.parse(
            await readFile(join(EVAL_RESULTS_DIR, name), "utf8"),
          ) as CoverageRow;
          coverage.push(row);
        } catch {
          /* skip malformed */
    ...
    coverage.sort(
      (a, b) => b.date.localeCompare(a.date) || a.arc.localeCompare(b.arc),
    );
