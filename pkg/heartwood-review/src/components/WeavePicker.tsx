...
import type { WeaveTarget, WeaveMode } from "@faerrin/heartwood/src/state/review.ts";
...
export function WeavePicker({
  ...
  const [anchorText, setAnchorText] = useState<string>(initial?.anchorText ?? "");
  ...
  return (
    <div style={{ marginBottom: "0.5rem", fontSize: "0.85rem", display: "flex", gap: "0.4rem", flexWrap: "wrap", alignItems: "center" }}>
      <span style={{ color: "#555" }}>Weave:</span>
      <select
        value={mode}
        onChange={(e) => setMode(e.target.value as WeaveMode)}
        style={{ font: "inherit", fontSize: "0.85rem", padding: "0.25rem", borderRadius: 6, border: "1px solid #ccc" }}
      >
      ...
      {mode !== "end" && (
        <select
          ...
          style={{ font: "inherit", fontSize: "0.85rem", padding: "0.25rem", borderRadius: 6, border: "1px solid #ccc", flex: 1, minWidth: 220 }}
        >
          {!paras && <option>loading paragraphs…</option>}
          {paras?.length === 0 && <option value="">(no prose paragraphs)</option>}
          {paras?.map((p, i) => (
