...
import { getWikiFolders, suggestInboundLinks, type InboundSuggestions } from "@/server/sessions";
...
function parseTargetPath(tp: string, fallbackName: string): { folder: string; filename: string } {
...
export function CreatePagePicker({
  ...
  useEffect(() => {
    void suggestInboundLinks({ data: { name: canonicalName } }).then(setInbound);
  }, [canonicalName]);
  useEffect(() => {
    ...
    <div style={{ marginBottom: "0.6rem", fontSize: "0.85rem" }}>
      <div style={{ display: "flex", gap: "0.4rem", alignItems: "center", flexWrap: "wrap" }}>
        ...
        <select
          ...
          style={{ font: "inherit", fontSize: "0.85rem", padding: "0.3rem", borderRadius: 6, border: "1px solid #ccc" }}
        >
        ...
        <input
          ...
          style={{ font: "inherit", fontSize: "0.85rem", padding: "0.3rem", borderRadius: 6, border: "1px solid #ccc", flex: 1, minWidth: 140 }}
        />
      ...
      {inbound &&
        (inbound.orphan ? (
          <div style={{ color: "#b06000", marginTop: "0.3rem" }}>
            ⚠ No existing page mentions “{canonicalName}” — this page will be an orphan. Consider
            linking it from a related page.
          </div>
        ) : (
          <details style={{ marginTop: "0.3rem" }}>
            <summary style={{ cursor: "pointer", color: "#2b6cb0" }}>
              {inbound.mentions.length} page(s) mention “{canonicalName}” (candidate inbound links)
            </summary>
