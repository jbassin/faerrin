...
// (mirrors content/scripts/lib/paths.ts and python/consts.py). Every path
// is overridable via a LISTENER_* env var for host cutover — see ../.env.example.
const here = path.dirname(fileURLToPath(import.meta.url)) // pkg/wretch/src
export const pkgRoot = path.resolve(here, "..") // pkg/wretch
