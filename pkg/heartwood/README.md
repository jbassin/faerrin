...
> Inputs are the monorepo SSOT in [`pkg/shared-content`](../shared-content): transcripts from
> `../shared-content/transcripts/`, the wiki from `../shared-content/wiki/` (excluding `Script/`).
...
- `ANTHROPIC_API_KEY` — LLM access (calls go through [`@faerrin/llm`](../faerrin-llm)).
