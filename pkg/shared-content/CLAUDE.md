# CLAUDE.md — `shared-content`

Guidance for **shared-content**: the monorepo's **content platform** — the single source of truth for
...
> *data* by filesystem path (cwd-relative `../shared-content/...`), never as a package import — the one
> exception is quartz's renderer importing `folderIndexName` from `scripts/lib/folder-index.ts`. Do
> not re-create per-app copies of `wiki/` or `transcripts/`.

## Data (the SSOT)

- **`wiki/`** — the hand-maintained Obsidian wiki (quartz is canonical for content). Also holds the
  **generated** `wiki/Script/` transcript pages (written by the pipeline's `export` step). quartz's
...
bun run --filter shared-content review            # transcript-correction review UI
```
...
- **`scripts/lib/`** — `paths`, `content` walker, `corrections`, `linker`, `campaigns`, `roster`,
  `http` (retry), `log`, `types`, and the **shared** `folder-index.ts` (also imported by quartz's
...
- **`scripts/lib/roster.ts`** — the speaker roster (recording user ID → display name + color); the
  SSOT that listener's `isPlayer` and the pipeline both use.
