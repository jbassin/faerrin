...
| `mouth` | `@faerrin/mouth` | **Rust** PF2e dice/host Discord bot (Cargo workspace + SQLite). A script-less Bun member (its Rust build/check is handled outside the bun CI lanes — like `gothic`; not gated in `ci.yml`). |
| `lark` | `@faerrin/lark` | Bun Discord **music** bot + Vite/React web library + Stream Deck API (`lark.iridi.cc`). One process: `@discordjs/voice` (pure-JS deps) playback engine, `bun:sqlite` library, yt-dlp/ffmpeg ingest. Separate bot/token from `mouth`. |
...
- **CI**: plain **GitHub Actions** (`.github/workflows/ci.yml`) — Dagger was removed. Each gate
  (`typecheck`, `astro-check`, `lint`, `test`, `build`, `visual-regression`) is its own **parallel**
  job; a shared composite (`.github/actions/setup`) pins Bun `1.3.14` and restores a bun dependency
  cache; `build`/`visual-regression` also cache Playwright Chromium. Reproduce locally with `bun run
  ci` (typecheck → check → lint → test → build). The **`visual-regression`** job runs inside the
  pinned `oven/bun:1.3.14` container because vellum's goldens were generated there — regenerate them in
  that same env with `bun --filter @faerrin/vellum visual-regression -- --update`. (`@faerrin/birdfeed`
  has its own publish workflow, `birdfeed-release.yml`.)
