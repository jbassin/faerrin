...
>
> **Implementation status (2026-06-09):** **fully implemented** on `main`. M0–M6 (`@faerrin/gothic`
> extracted + strider consuming it; renderer library; editor SPA; warm Playwright render service with
> the full SEC-1…5 model; six-kind zoo + mechanical|diegetic axis; slash palette / gallery / share
> links / seeded grime; Caddy + systemd deploy), **plus** the two follow-ups: the full multi-document
> manager (new/switch/rename/delete — completes R-19) and golden-image visual-regression in Dagger CI
> (NFR-9 — container-authoritative goldens; a `visual-regression` CI job + `update-goldens` regen
> path; `bun test` stays pure so the unit-test job needs no browser).
